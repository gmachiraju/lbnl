//Functions referenced from Numerical Recipes  -- Assembled by Gautam Machiraju
...
int main() {
    ...
    int n2 = 0;
	ifstream r2 ("RatioFile2"); //raw data points
	std::string unused;
    ...
    int n3 = 0;
	ifstream r3 ("RatioFile3"); //binned values
	std::string unused;
	while ( std::getline(in, unused) ) {
   		++n3;
   	}
   	std::ifstream x3("x_RatioFile3", std::ios::binary);
	std::vector<double> xv3((std::istreambuf_iterator<char>(x3)), std::istreambuf_iterator<char>());
	std::ifstream y3("y_RatioFile3", std::ios::binary);
	std::vector<double> yv3((std::istreambuf_iterator<char>(y3)), std::istreambuf_iterator<char>());


    // Now you have 4 arrays: xv2, xv3, yv2, and yv3 - The interpolation routine needs vecDoub objects
    VecDoub xvd2(num, xv2);
    VecDoub yvd2(num, yv2);
    VecDoub xvd3(num, xv3);
    VecDoub yvd3(num, yv3);
